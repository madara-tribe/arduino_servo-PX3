...
void setSTA(){
  ...
  // ---- Like wifi_connect.ino: STA mode + begin ----
  Serial.print("WiFi(STA) try connect SSID: ");
  Serial.println(STA_SSID);

  WiFi.mode(WIFI_STA);
  WiFi.setSleep(false);          // helps stability for some routers
  WiFi.disconnect(true);         // clean start (optional but useful)
  delay(200);
...
// ===== ESP-NOW callbacks (Arduino-ESP32 core 2.x/3.x compatible) =====
